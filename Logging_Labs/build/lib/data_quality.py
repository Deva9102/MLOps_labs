...
def analyze_missing_values(df: pd.DataFrame) -> Dict[str, int]:
    """
    Count missing values per column and log warnings for columns
...
def analyze_numeric_ranges(df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    """
    Look at numeric columns and log warnings for negative values.
    """
...
def basic_schema_summary(df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    """
    Provide a very simple schema-like summary:
      - dtype
      - approx number of unique values
    """
...
def analyze_constant_columns(df: pd.DataFrame) -> List[str]:
    ...
    These are often not useful for modeling.
    """
...
def analyze_high_cardinality_categoricals(
    ...
    In a real system, 'threshold' might be 1000+.
    For our small demo dataset, we will call this with a lower threshold
    from run_quality_checks so it actually triggers.
    """
...
def check_value_ranges(
    ...
    Logs an 'out_of_range' warning if values fall outside the range.
    """
...
def run_quality_checks(path: str | Path) -> Dict[str, Any]:
    """
    Run all quality checks on the given dataset path.

    Returns a Python dict summary so callers can print it nicely
    or save it as JSON.
    """
    ...
    # For demo, use a lower threshold so user_id gets flagged
